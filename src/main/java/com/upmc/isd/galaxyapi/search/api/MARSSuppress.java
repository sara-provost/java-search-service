...
@Path("/marssuppress")
public class MARSSuppress {

	@POST
	@Produces(MediaType.APPLICATION_JSON)
	@Consumes(MediaType.APPLICATION_JSON)
	
	public Response doSuppressAction(MARSCollectionSearch search){
		SolrOperation operation = new SolrOperation.Builder(search).build();
		
		if(!operation.getAction().equalsIgnoreCase("SUPPRESS") || !operation.getAction().equalsIgnoreCase("UNSUPPRESS")){
